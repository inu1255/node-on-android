...
import cn.inu1255.soulsign.proxy.dns.Question;
import cn.inu1255.soulsign.proxy.dns.Resource;
import cn.inu1255.soulsign.proxy.dns.ResourcePointer;
    ...
    private boolean interceptDns(IPHeader ipHeader, UDPHeader udpHeader, DnsPacket dnsPacket) {
        ...
        if (question.Type == 1) {
            if (ProxyConfig.Instance.needProxy(question.Domain, getIPFromCache(question.Domain))) {
                ...
                if (ProxyConfig.IS_DEBUG)
                    Log.d(Constant.TAG, "interceptDns FakeDns: " + question.Domain + " " + CommonMethods.ipIntToString(fakeIP));
